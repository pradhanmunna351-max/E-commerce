  ...
  const results = useMemo(() => {
    ...
    return activeData.map(row => {
      ...
      if (currentMarketplace === Marketplace.AJIO) {
        ...
        const aisp = findAISPForTarget(target, level, row.articleType, false, Region.LOCAL, ReverseLogisticsMode.FIXED, 0, currentMarketplace, row.brand, undefined, undefined, manualRateCard);
        breakdown = calculateBreakdown(aisp, level, row.articleType, false, Region.LOCAL, ReverseLogisticsMode.FIXED, 0, currentMarketplace, row.brand, undefined, undefined, manualRateCard);
